...
namespace BM.API.Controllers
{
    ...
    public class DocumentController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("Config")]
        public async Task<IActionResult> ConfigUpdate([FromBody] RequestModel request)
        {
            try
            {
                var response = await _documentervice.ConfigUpdate(request);
                if (response == null || response.StatusCode != 0) return StatusCode(StatusCodes.Status400BadRequest, new
                {
                    StatusCode = StatusCodes.Status400BadRequest,
                    Message = response?.Message ?? "Vui lòng liên hệ IT để được hổ trợ."
            ...
            {
                _logger.LogError(ex, "DocumentController", "ConfigUpdate");
                return StatusCode(StatusCodes.Status400BadRequest, new
                {
                    StatusCode = StatusCodes.Status400BadRequest,
                    ex.Message
